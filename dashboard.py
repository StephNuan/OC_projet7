...
with st.sidebar:
    ...
    show_client_details = st.checkbox("Informations du client")
    show_client_comparison = st.checkbox("Comparaison aux autres clients")
    shap_general = st.checkbox("Feature importance")
...
html_temp = """
    <div class="st-d1" style="padding: 10px; border-radius: 10px; text-align: center;">
    <h1 style="color: #0055A4;">Dashboard de Scoring Crédit</h1>
    </div>
    <p style="font-size: 20px; text-align:center">
    Support de décision de crédit </p>
    """
st.markdown(html_temp, unsafe_allow_html=True)


#Afficher l'ID Client sélectionné
st.write("ID Client :", id_client)
...
if show_credit_decision:
    ...
    if classe_predite == 1:
        etat = 'Client à risque élevé'
    else:
        etat = 'Client à risque faible'
...
if (show_client_details):
        ...
        with st.spinner('Chargement des informations client...'):
            ...
            show_all_info = st.checkbox("Visualisation sous forme de tableau")
            if (show_all_info):
...
if (shap_general):
        ...
        ax.set(title='Importance des features', xlabel='', ylabel='')
        st.pyplot(fig)  
